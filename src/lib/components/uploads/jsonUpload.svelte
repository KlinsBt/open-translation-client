<script lang="ts">
	...
	import { generateJsonTranslation } from "$lib/functions/outputGeneration/outputGenerationJson";
	import {
		showLoading,
...
>
	...
	{#if values.length === 0 || temporarySaveName.length === 0 || sourceLanguage.length === 0 || targetLanguage.length === 0}
		<DragAndDropHere />
		<button class="input-button-container">
			...
			<input
				...
				oninput={handleJsonTranslationFileUpload}
			/>
		</button>
	{:else}
	...
	<!-- <button onclick={() => generateJsonTranslation($translationIdSelected)}>
		Download Translation in JSON Format
	</button> -->
</div>
